<template>
  <div
    class="EasyNormalTable"
    element-loading-spinner="el-icon-loading"
    style="height: calc(100vh - 60px - 2rem)"
  >
    <el-card class="box-card">
      ...
      <div align="right" class="filter-container" v-if="titleShow">
        <span class="Title_front main_color" style="float: left"
          >{{ $t(title) }}{{ $t("table.detail") }}
          <el-popover
          ...
          >
            <div
              v-html="helpContent"
              class="el-popover__Content_customize"
            ></div>
            <i
        ...
        <el-input
          :placeholder="defaultSerchTooltip"
          @input="inputChange"
          class="filter-item"
          style="width: 25%; vertical-align: top"
          v-bind:prefix-icon="changeIcon"
          v-model="searchValue"
        >
        </el-input>
      </div>
      ...
      >
        ...
        >
          <el-table-column
            :key="o.code"
            :label="$t(o.label)"
            :label-class-name="o.labelClass"
            v-for="o in item.child"
            v-if="o.child && o.child.length > 0"
          >
            <el-table-column
              :column-key="oo.code"
              ...
              :key="oo.code"
              :label="$t(oo.label)"
              :label-class-name="oo.labelClass"
              :min-width="oo.width"
              :prop="oo.code"
              align="left"
          ...
          <el-table-column
            :column-key="o.code"
            :filters="o.filter === true ? filtersdata(o) : null"
            :fixed="o.fix"
            :formatter="formatter"
            :key="o.code"
            :label="$t(o.label)"
        ...
        <el-table-column
          ...
          :filters="item.filter === true ? filtersdata(item) : null"
          :fixed="item.fix"
          :formatter="formatter"
          :key="item.code"
          :label="$t(item.label)"
        ...
        <!--        add-ws-12/21-印章盖印-->
        <el-table-column
          :label="$t('label.PFANS4001FORMVIEW_ACCEPTSTATE')"
          align="center"
          width="110vw"
          v-if="handles"
        >
        ...
        <el-table-column
          :label="$t('label.PFANS4001FORMVIEW_REGULATORSTATE')"
          align="center"
          v-if="handles"
          width="130vw"
        >
      ...
      <div class="pagination-container" style="padding-top: 20px">
        <el-pagination
          ...
          :page-sizes="[10, 50, 100, 500, 99999]"
          :total="total"
        ...
        >
          <slot
            ><span
              ...
              >{{ $t("table.total") }}{{ totaldata.length }}</span
            >
          </slot>
        </el-pagination>
      </div>
...
export default {
    ...
    buttonList: {
      ...
      default: function () {
        return [
          {
            key: "new",
            name: this.$t("button.insert"),
            disabled: false,
            icon: "el-icon-plus",
          },
          {
            key: "update",
            name: this.$t("button.update"),
            disabled: false,
            icon: "el-icon-edit",
          },
        ];
  ...
  created() {
    ...
    let Content = helpContent().filter(
      (item) => item.id == this.$router.currentRoute.name
    );
    if (Content.length > 0) {

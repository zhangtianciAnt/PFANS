  ...
  <div element-loading-spinner="el-icon-loading" style="height: calc(100vh - 60px - 2rem)">
    <el-card>
      <el-collapse v-model="activeNames">
        <el-collapse-item v-for="(list,index) in flowList" :key="index" :name="index">
          <template slot="title">
            <span class="collapse_Title">{{ list.title }}</span>
          </template>
          <el-row align="middle" type="flex">
            <el-col v-for="item in list.flow" :key="item.title" :span="4" class="col">
              <el-col class="col">
                <el-button :disabled="disabled(item.name)" size="mini" style="font-size: 1px" @click="openUrl(item)">
                  {{ item.title }}
                </el-button>
              </el-col>
              <el-col class="col">
                <i v-if="item !== list.flow[list.flow.length-1]" class="el-icon-right"></i>
              </el-col>
            </el-col>
          </el-row>
        </el-collapse-item>
...
export default {
  ...
  methods: {
    inint() {
      ...
      } else if (type === 'FINANCIAL' || type === 'MANAGEMENT') {

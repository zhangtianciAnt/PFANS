<template>
  <div>
    <EasyNormalTable ref="roletable" v-loading="loading" :buttonList="buttonList" :columns="columns"
                     :data="data" :title="title" @buttonClick="buttonClick">
      <el-tag v-if="this.resultShow" slot="customize" style="float: left;margin-left: 3vw;font-size:15px;margin-top:-4px"
              type="success">
        {{ $t('label.PFANS2017VIEW_WORKHOURS') }}&nbsp;&nbsp;&nbsp;&nbsp;{{ this.workHours }}
      </el-tag>
    </EasyNormalTable>
  </div>
</template>

<template>
  <div>
    ...
    <PFANS2016Pop ref="PFANS2016Pop" :params="urlparams" :url="url"></PFANS2016Pop>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    if (this.params_id) {
      this.loading = true;
      this.$store
        .dispatch('PFANS2010Store/getabnormalByuseridandDate', {
          'user_id': this.params_id,
          'dates': this.$route.params._dates,
        })
        .then(response => {

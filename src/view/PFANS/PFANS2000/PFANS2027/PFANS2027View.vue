<template>
  <div>
    <EasyNormalTable v-loading="loading" :buttonList="buttonList" :columns="columns" :data="data" :rowid="row_id"
                     :title="title"
                     @buttonClick="buttonClick" @reget="get" @rowClick="rowClick">
    </EasyNormalTable>
    <el-dialog :visible.sync="dialogFormVisible" @closed="close">
      <el-form ref="refform" v-loading="loading" :model="form" :rules="rules"
               label-position="top" label-width="8vw" style="padding: 2vw">
        <el-row>
          <el-col :span="8">
            <el-form-item :label="$t('label.PFANS2007VIEW_YEAR')" prop="evaluationday">
              <el-date-picker v-model="form.evaluationday" style="width: 10rem" type="year"></el-date-picker>
            </el-form-item>
          </el-col>
          <el-col :span="8">
...
import {getCurrentRole10, getDictionaryInfo, getStatus} from '@/utils/customize';

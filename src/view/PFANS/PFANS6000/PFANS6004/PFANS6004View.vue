    ...
    buttonClick(val) {
      ...
      if (val === 'crAccount2') {
        ...
        if (this.$refs.roletable.selectedList.length === 0) {
          ...
          let seleList = this.$refs.roletable.selectedList;
          for (let a = 0; a < seleList.length; a++) {
            if (seleList[a].accountname !== null && seleList[a].accountname !== '' && seleList[a].accountname !== undefined) {
              Message({
                message: this.$t('label.PFANS6004VIEW_CRACCOUNTERRORIN') + this.$t(seleList[a].expname) + this.$t('label.PFANS6004VIEW_CRACCOUNTERRORON'),
                type: 'error',
                duration: 2 * 1000,
              });
              return;
      ...
      if (val === "reset") {
        ...
        this.$confirm(this.$t('normal.confirm_resetPassword') + this.rowname + this.$t('normal.confirm_password'), this.$t('normal.info'), {

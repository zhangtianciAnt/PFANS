    ...
    methods: {
      changeRegion(val) {
        ...
        if (this.form.group_id && this.form.year && this.form.region) {
          ...
          //add-ws-5/6-添加按份金额
          this.getList2(this.form.group_id, this.form.year, this.form.region);
          //add-ws-5/6-添加按份金额
        } else {
      ...
      getById() {
        ...
        //add-ws-5/6-添加按份金额
        this.getList2(this.optionsdata[0].value, this.form.year, this.form.region);
        //add-ws-5/6-添加按份金额
        //add_fjl
        this.loading = false;
      },
...
//共通PJ（研修会议等）
      ...
      //add-ws-5/6-添加按份金额
      getList2(groupid, year, month) {
        this.loading = true;
        this.$store
          .dispatch('PFANS1042Store/selectPlmoney', {'groupid': groupid, 'year': year, 'month': month})
          .then(response => {

<template>
  <div>
    <EasyNormalTable v-loading="loading" :buttonList="buttonList" :columns="columns" :data="data" :rowid="row" :title="title"
                     @buttonClick="buttonClick" @reget="load" @rowClick="rowClick">
    </EasyNormalTable>
    <el-container>
...
import {getDictionaryInfo, getMonthlyrateInfo, getStatus, getUserInfo} from '@/utils/customize';
...
export default {
  ...
  methods: {
    ...
    load() {
      this.loading = true;
      this.$store
        .dispatch('PFANS1026Store/getPage', {'type': '1'})
        .then(response => {

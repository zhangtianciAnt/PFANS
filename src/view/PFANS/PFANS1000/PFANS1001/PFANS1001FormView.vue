      ...
      getBudt(val) {
        ...
        if(getOrgInfo(val)){
          ...
          if(this.new_options.length === 0){
            ...
            if (!getOrgInfo(this.form.new_group_id) || !getOrgInfo(this.form.new_group_id).encoding) {//如果没有group,或者encoding为空--经营管理
              coding = "100000";//人事总务coding
            } else {
              coding = getOrgInfo(this.form.new_group_id).encoding;//正常各部门coding
            }

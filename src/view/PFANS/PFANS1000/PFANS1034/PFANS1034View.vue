  ...
  <div class="EasyNormalTable" style="min-height: 100%">
    <el-card class="box-card">
      ...
      <div align="right" class="filter-container" style="padding-bottom: 10px">
        ...
        <el-input v-model="searchValue" :placeholder="defaultSerchTooltip" class="filter-item"
                  style="width: 25%;vertical-align:top" v-bind:prefix-icon="changeIcon" @input="inputChange">
        </el-input>
      </div>
      <el-table ref="eltable" v-loading='loading' :data="this.pagedate"
                :default-sort='defaultSort'
                :element-loading-text="$t('normal.waiting')" :row-key="rowid" :tree-props="{children: 'children', hasChildren: 'hasChildren'}"
                border default-expand-all
                header-cell-class-name="sub_bg_color_grey height" header-row-class-name="height" height="400"
                highlight-current-row max-height="400" stripe
                style="width: 100%" @filter-change="tableFilter" @row-click="rowClick" @row-dblclick="rowClick" @selection-change="handleSelectionChange"
                @sort-change="sortChange">>
        <el-table-column v-if="showSelection" reserve-selection type="selection" width="55">
        </el-table-column>
      ...
      <div class="pagination-container" style="padding-top: 20px">
        <el-pagination :current-page.sync="listQuery.page" :page-size="listQuery.limit"
                       :page-sizes="[10,20,30,50]" :total="total" layout="slot,sizes, ->,prev, pager, next, jumper"
...
import {downLoadUrl, orderBy, uploadUrl} from '@/utils/customize';

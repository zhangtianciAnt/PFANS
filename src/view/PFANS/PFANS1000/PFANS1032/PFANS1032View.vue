...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getClaDate(val) {
        if(val !== null) {
          this.retral.claimdate = moment(val).format("YYYY-MM-DD");
        } else {
          this.retral.claimdate = null
        }
      },
      changeType(val) {
      ...
      getList() {
        this.loading = true;
        this.$store
          .dispatch('PFANS1032Store/getPetSearch', this.retral)
          .then(response => {

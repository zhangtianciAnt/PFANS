          ...
          <el-tabs v-model="activeName" type="border-card">
            <el-tab-pane :label="$t('label.PFANS5009FORMVIEW_PROJECTINFORMATION')" name="first">
              ...
              <el-row>
                ...
                <el-col :span="8">
                  <el-form-item :label="$t('label.PFANS5001FORMVIEW_FIELD')" prop="field">
                    <dicselect
                      :code="code1"
                      :data="form.field"
                      :disabled="true"
              ...
              <el-row>
                <el-col :span="8">
                  <el-form-item :label="$t('label.PFANS5001FORMVIEW_COUNTRY')" prop="country">
                    <dicselect
                      :code="code6"
                      :data="form.country"
                      :disabled="true"
                ...
                <el-col :span="8">
                  <el-form-item :label="$t('label.PFANS5001FORMVIEW_CARON')" prop="caron">
                    <dicselect
                      :code="code7"
                      :data="form.caron"
                      :disabled="true"
                ...
                <el-row>
                  <el-col :span="24">
                    ...
                    >
                      ...
                      <el-table-column
                        :label="$t('label.PFANS5009FORMVIEW_PRODUCTSTATUS')"
                        align="center"
                        width="100"
                      >
                        <template slot-scope="scope">
                      ...
                      <el-table-column
                        :label="$t('label.PFANS5009FORMVIEW_ACTUALWORK')"
                        align="center"
                        width="150"
                      >
                        <template slot-scope="scope">
                      ...
                      <el-table-column
                        :label="$t('label.PFANS5009FORMVIEW_ACTUALSTARTTIME')"
                        align="center"
                        width="200"
                      >
                        <template slot-scope="scope">
                      ...
                      <el-table-column
                        :label="$t('label.PFANS5009FORMVIEW_ACTUALENDTIME')"
                        align="center"
                        width="200"
                      >
                        <template slot-scope="scope">
                      ...
                      <el-table-column
                        :label="$t('label.PFANS5009FORMVIEW_PRODUCT')"
                        align="center"
                        width="200"
                      >
                        <template slot-scope="scope">
    ...
    methods: {
      ...
      changecontract(row) {
        ...
        for(let i = 0; i < this.tableD.length; i++){
          if(!this.tableD[i].workinghours){
            continue;
          }else{
            this.time[i] = this.tableD[i].workinghours;
          }
        }
        //获取当前表中已有合同回数的时间 scc
        ...
        this.$store
          ...
          .then(response => {
            ...
            if(contradeta.length > 0){
                let time = contradeta[0].contractdate;//字符串
                let extensdate = contradeta[0].extensiondate;
                if(extensdate){
                  let time1 = time.split('~');//数组
            ...
            for(let i = 0; i < this.time.length; i++){
              //每条回数的时间
              let date3 = new Date(this.time[i][0]);
              let date4 = new Date(this.time[i][1]);
              //判断新添加合同的两个时间点，在不在已有合同回数时间的区间内
              let e1 = ((date1 > date3 && date1 < date4) || (date2 > date3 && date2 < date4));
              //判断新添加合同的两个时间点，与已有合同回数的时间点是否重合
              let e2 = (date1 === date3 || date1 === date4 || date2 === date3 || date2 === date4);
              //判断新添加合同的两个时间点，是不是包含或者被包含于已有合同回数时间
              let e3 = ((date1 < date3 && date2 > date4) || (date1 > date3 && date2 < date4));
              //判断上述条件是否为真
              let e4 = (e1 || e2 || e3 );
              if(e4){
                      Message({
